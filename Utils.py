...
def plotEMF(numRelFreqPairs, force_display = True):
    ...
    if (force_display):
        plt.show()
'''
Plots the Empirical Mass Function (EMF) given a list of number-relative frequency pairs.
numRelFreqPairs: A list or array of pairs where the first element is the value and the second element is the relative frequency.
force_display: Whether to force display the plot (default is True).
The empirical distribution is the distribution function of a discrete variable
numRelFreqPairs = [(1, 0.2), (2, 0.3), (3, 0.5)]
plotEMF(numRelFreqPairs)
'''
def makeFreq(data_sequence):
    ...
    Calculates the frequency of unique values in a data sequence.
    data_sequence = [1, 1, 2, 3, 3, 3, 4, 5, 5]
    freq = makeFreq(data_sequence)
    print(freq)
    [[1 2]
    [2 1]
    [3 3]
...
def makeEDF(data_sequence):
    '''
	Calculates the Empirical Distribution Function (EDF) for a data sequence.
...
x0 = 7
n = 10
random_sequence = linConGen(m, a, b, x0, n)
...
def classification_report_interval(
    ...
    Returns:
    a printable string.
    Produces a classification report with precision, recall, and accuracy, including confidence intervals for each metric using Hoeffding’s inequality.
    y_true = np.array([0, 1, 0, 1, 0, 1, 1])
y_pred = np.array([0, 0, 0, 1, 0, 1, 1])
report = classification_report_interval(y_true, y_pred, alpha=0.05)
print(report)
    """
    import numpy as np
...
# Split the data
X_train, X_test, X_valid, Y_train, Y_test, Y_valid = train_test_validation(X, Y, test_size=0.2, validation_size=0.2)
